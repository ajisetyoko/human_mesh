...
def draw_skeleton(input_image, joints, draw_edges=True, vis=None, radius=None):
    ...
    for child in range(len(parents)):
        ...
        if vis is not None and vis[child] == 0:
            continue
        if draw_edges:
            cv2.circle(image, (point[0], point[1]), radius, tuple([int(x) for x in colors['white']]),-1)
            # cv2.circle(image, (point[0], point[1]), radius - 1, colors[jcolors[child]], -1)
            cv2.circle(image, (point[0], point[1]), radius - 1, tuple([int(x) for x in colors[jcolors[child]]]), -1)
        else:
        ...
        if draw_edges and pa_id >= 0:
            ...
            cv2.circle(image, (point_pa[0], point_pa[1]), radius - 1, tuple([int(x) for x in colors[jcolors[pa_id]]]), -1)
            if child not in ecolors.keys():
                print('bad')
                import ipdb
                ipdb.set_trace()
            #cv2.line(image, (point[0], point[1]), (point_pa[0], point_pa[1]),colors[ecolors[child]], radius - 2)
            cv2.line(image, (point[0], point[1]), (point_pa[0], point_pa[1]),tuple([int(x) for x in colors[ecolors[child]]]), radius - 2)
